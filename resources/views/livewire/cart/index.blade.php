...
<div>
    ...
    <section x-data="carritoMain()" class="relative">
        <div class="relative">
            <button @click="animate = (animate) ? false : true" class=" bg-white rounded-full text-gray-500 hover:text-gray-800 transform hover:scale-110">
                @if (session()->has('carrito') && count(session('carrito')) > 0)
                    <div class="rounded-full bg-red-600 py-1 px-2 text-xs text-white font-bold absolute right-5">
