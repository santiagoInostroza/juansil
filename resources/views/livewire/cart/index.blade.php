...
<div x-data="{animate: {{$openCarrito}} } " class="relative" >
    ...
    <div class="card fixed z-40 top-0 w-screen sm:w-max-content right-0 h-screen hidden" :class="{hidden: !animate}"  {{--@click.away="animate = false"--}} x-show="animate" x-transition:enter="transition ease-out duration-1000" x-transition:enter-start="opacity-0 transform scale-90"  x-transition:enter-end="opacity-100 transform scale-100"  x-transition:leave="transition ease-in duration-1000"  x-transition:leave-start="opacity-100 transform scale-100"  x-transition:leave-end="opacity-0 transform scale-90">
