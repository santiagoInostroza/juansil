<div class="card  mt-5" x-data="{open:false}">
    ...
    @else
        <div class="card-header">
            <h2 x-on:click="open=!open">Entregados ({{ $ventas_entregadas }})  Total entregado ${{ number_format($ventas->sum('total'),0,',','.')}}</h2>
        </div>

        <div class="card-body" class="hidden" :class="{'hidden':!open}">
