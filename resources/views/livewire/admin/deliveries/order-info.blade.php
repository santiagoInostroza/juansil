                ...
                <div class="h3 form-group pb-0 mb-0 d-flex" style="justify-content: space-between; align-items: center;">
                    ...
                    @if ($venta->customer->celular)
                        <div class="w-full flex items-center gap-4 text-3xl" >
                            ...
                            <a href="https://api.whatsapp.com/send?phone={{ $venta->customer->celular }}&text=Hola,%20Le%20envío%20datos.%20Cuenta%20Rut%20Patricia%20Arias%2016.720.449-k%20patriciaariasolivares27@gmail.com"
                                target="_blank"><i class="fab fa-whatsapp p-2 bg-success"></i></a>
        ...
        <div>
            <div class="relative" x-data="{loading:false}" id="pay_{{$venta->id}}">
                ...
                <div class="bg-green-500 text-white p-1 rounded">
                    <span> Pagado el {{ Helper::fecha($venta->payment_date)->dayName}} {{ Helper::fecha($venta->payment_date)->format('H:i') }} </span><i class="fas fa-check"></i>
                </div>
        ...
        <div class="relative" x-data="{loading:false}" id="deliver_{{$venta->id}}">
            ...
            <div class="flex justify-between items-center"  >
                ...
                @elseif($venta->delivery_stage==1) {{-- PAGADO --}}
                    <div class="bg-green-500 text-white p-1 rounded">
                        <span> Entregado  el {{ Helper::fecha($venta->date_delivered)->dayName}} {{ Helper::fecha($venta->date_delivered)->format('H:i') }} </span><i class="fas fa-check"></i></span>
                    </div>
