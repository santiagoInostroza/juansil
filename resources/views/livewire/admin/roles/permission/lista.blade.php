<div>
    ...
    <div>
        {{-- TITULO --}}
        <h1 class="p-4 text-center uppercase text-3xl text-gray-600 font-bold my-4">Lista de permisos</h1>
        {{-- SEARCH --}}
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="ml-4">
                                                ...
                                                {{-- ALPINE --}}
                                                <div x-data="{editButton:false, openEdit:false, item_id:'',description:'',loading:false}" x-init="item_id={{$item->id}}; description='{{$item->description}}'" x-on:mouseenter="editButton=true" x-on:mouseleave="editButton=false" class="relative rounded hover:bg-gray-200">
                                                    <div class="text-sm font-medium text-gray-900 p-2">
                                                        {{$item->description}}
                                                    </div>
                                                    <div x-show="editButton" x-on:click="openEdit = true" class="absolute p-2 px-3 top-0 right-0 transform translate-x-4 bg-gray-200 cursor-pointer"   ><i class="fas fa-pen"></i></div>
                                                    <div x-show="openEdit">
                                                        <x-modal.modal2>
                                                            <div class="p-4 px-2">
                                                                ...
                                                                <x-jet-input x-model="description"> {{$item->description}}</x-jet-input>
                                                                {{-- <x-dropdowns.dropdown2  :items="$items" id="select_item_{{$item->id}} "></x-dropdowns.dropdown2> --}}
                                                            </div>
                                                            <div class="p-4 px-2">
                                                                <x-jet-button x-on:click="openEdit=false; loading= true; $wire.editDescription(item_id,description).then( ()=> loading=false )">Editar</x-jet-button>
                                                                {{-- <x-jet-button x-on:click="openEdit=false; loading= true; $wire.editName(item_id,document.getElementById('select_item_{{$item->id}}_value').value).then( ()=> loading=false )">Editar</x-jet-button> --}}
                                                           </div>
                                                        </x-modal.modal2>
                                                    </div>
    ...
    <div>
        ...
        <div>
            <x-modal.modal2> 
                <div class="p-4 px-2">
                    <div class="flex justify-between items-center gap-4 mb-4">
                        <h2 class="font-bold text-xl text-gray-600">Crear Permiso</h2>
